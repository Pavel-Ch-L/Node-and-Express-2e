...
// Конфигурация Handlebars view engine
...
const auth = createAuth(app, {
	// baseUrl опционален; По умолчанию будет использоваться localhost;
    baseUrl: process.env.BASE_URL,
    providers: credentials.authProviders,
    successRedirect: '/account',
    failureRedirect: '/unauthorized',
})
// auth.init() Подключается промежуточное ПО Passport:
auth.init()
// Теперь можем указать маршруты:
auth.registerRoutes()
...
const employeeOnly = (req, res, next) => {
  ...
  // Пропустит маршрут в целом, что приведет к обработчику 404
  next('route')
}
