// Демонстрация работы шаблона письма views/email/cart-thank-you.hbs
...
app.post('/cart/checkout', (req, res, next) => {
  ...
  // Передача параметром функцию обратного вызова препятствует рендеренгу результатов представления.
  //   Взамен функция обратного вызова получает отображенное представление в параметре html.
  //   Второй вызов line-73 res.render('cart-thank-you', { cart: cart }) работает как обычно.
    (err,html) => {
